from chemistry import init_periodic_table, \
...
def test_molar_mass():
    ...
    assert molar_mass([], periodic_table) == 0
    assert molar_mass([["O",2]], periodic_table) == approx(31.9988)
    assert molar_mass([["C",6],["H",6]], periodic_table) == approx(78.11184)
    assert molar_mass([["C",13], ["H",16], ["N",2], ["O",2]], periodic_table) == approx(232.27834)


pytest.main(["-v", "--tb=line", "-rN", "test_chemistry.py"])